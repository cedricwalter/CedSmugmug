...
class plgContentSmugmugRandom extends JPlugin
{
	function plgContentSmugmugRandom( &$subject, $params )
	{
		...
		$this->debug= intval($pluginParams->get('debug','1'));
		$this->demo= intval($pluginParams->get('demo','1'));

		$this->defaultUrl = $pluginParams->get('defaultUrl', 'http://cedricwalter.smugmug.com');

		$this->defaultId = $pluginParams->get('defaultId', '2504559');
		$this->defaultKey = $pluginParams->get('defaultKey', 'f3ta9');
		$this->defaultLimit = intval($pluginParams->get('defaultLimit', '5'));
		$this->defaultSize = $pluginParams->get('defaultSize', '150x150');
		$this->defaultCols = intval($pluginParams->get('defaultCols', 3));
		$this->defaultText = $pluginParams->get('defaultText', "Go to Gallery");
