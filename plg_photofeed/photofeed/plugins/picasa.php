...
class PhotoFeedPicasaPlugins extends PhotoFeedPlugins
{

    function PhotoFeedPicasaPlugins($config = array())
    {
    ...
    {
        $picasaSize = $params->get('picasaSize', 0);

        $thumbnailUrl = "";
        foreach ($item->get_enclosures() as $enclosure) {
            $thumbnails = $enclosure->get_thumbnails();
            if ($thumbnails != null) {
              $thumbnailUrl = $thumbnails[$picasaSize];
            }
        }
